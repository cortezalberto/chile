...
def _build_ticket_output(
    ...
    CRIT-02 FIX: Uses pre-loaded relationships when available to prevent N+1.
    Falls back to database query only when relationships not loaded.
    """
    item_outputs = []

    try:
        # CRIT-02 FIX: Try to use pre-loaded items first (from selectinload)
        if hasattr(ticket, "items") and ticket.items is not None:
            # Use pre-loaded items - no additional queries
            for ticket_item in ticket.items:
                # Try to get product from pre-loaded relationship
                product_name = "Unknown"
                product_id = 0
                notes = None

                if hasattr(ticket_item, "round_item") and ticket_item.round_item:
                    round_item = ticket_item.round_item
                    product_id = round_item.product_id
                    notes = getattr(round_item, "notes", None)

                    if hasattr(round_item, "product") and round_item.product:
                        product_name = round_item.product.name
                    elif hasattr(ticket_item, "product") and ticket_item.product:
            ...
            # Fallback: Query database if relationships not loaded
            # This should rarely happen with proper eager loading
            logger.debug(
                "Falling back to database query for ticket items",
                ticket_id=ticket.id,
            )
...
def list_pending_tickets(
    ...
    # CRIT-02 FIX: Build query with comprehensive eager loading to prevent N+1 queries
    # Loads: ticket -> items -> round_item -> product
    query = (
...
def get_ticket(
    ...
    CRIT-02 FIX: Uses comprehensive eager loading to prevent N+1 queries.

    Requires KITCHEN, WAITER, MANAGER, or ADMIN role.
    """
    require_roles(ctx, ["KITCHEN", "WAITER", "MANAGER", "ADMIN"])

    # CRIT-02 FIX: Single query with all necessary joins
    ticket = db.scalar(
        select(KitchenTicket)
        .options(
            selectinload(KitchenTicket.items)
    ...
    # CRIT-02 FIX: Get round with session and table in single query
    round_obj = db.scalar(
        select(Round)
        .options(
            joinedload(Round.session).joinedload(TableSession.table)
    ...
    # Extract session and table from pre-loaded relationships
...
async def update_ticket_status(
    ...
    # HIGH-07 FIX: Validate status transition using constants
    valid_transitions = {
    ...
    # HIGH-07 FIX: Validate new status is valid
    if not validate_ticket_status(new_status):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Invalid ticket status: {new_status}",
        )
    ...
    # HIGH-07 FIX: Use constants instead of string literals
    if new_status == TicketStatusConst.IN_PROGRESS:
        ...
        db.execute(
            ...
            .values(status=TicketItemStatusConst.IN_PROGRESS)
        )
        ...
        db.execute(
            KitchenTicketItem.__table__.update()
            .where(KitchenTicketItem.ticket_id == ticket_id)
            .values(status=TicketItemStatusConst.READY)
        )
