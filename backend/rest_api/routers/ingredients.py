...
def list_ingredient_groups(
    ...
    groups = db.execute(
        ...
        .where(IngredientGroup.is_active == True)
...
def create_ingredient_group(
    ...
    existing = db.scalar(
        select(IngredientGroup).where(
            ...
            IngredientGroup.is_active == True,
        )
    )
    if existing:
        raise HTTPException(
...
def list_ingredients(
    ...
    ingredients = db.execute(query.order_by(Ingredient.name)).scalars().all()
